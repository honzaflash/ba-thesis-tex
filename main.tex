...
\begin{itemize}[--]
    ...
    We use the predifined \inlinehs{instance Monoid a => Applicative ((,) a)},
...
\mintinline{console}{wc --help}: \say{A word is a non-zero-length sequence
...
\begin{itemize}[\indent]
    ...
    However, the build was tested only on Ubuntu~20.04 and Windows~10 build~19042.
\end{itemize}

\noindent With that, we can run \mintinline{console}{stack build} in the game's root directory.
