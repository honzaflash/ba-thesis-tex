...
referencing back to the promised benefits of Haskell from section \ref{sect:whyfpmatters}.
Then we discuss the conflicts observed in the comparisons and finally summarize our findings.
...
imperative languages represented by \cpp{}. Keep in mind that \cpp{} is by no means meant
to represent imperative languages fully, even less so is the chosen imperative implementation
of Asteroids (further referred to as imperative Asteroids) meant to represent
the ideal of a game in \cpp{}. We try to address this in especially relevant places,
...
The section is then closed off with few statistical observations.
...
side effects was actually the plan B in this case. We saw how this works out well for expressiveness and clarity
...
correctness of a program is easier to perceive and argue about.
...
The already discussed Haskell features all bring the costs down but there is more,
...
from imperative asteroids and pure-asteroids.\footnote{
...
Due to time constraints and the simple nature of Asteroids we did not use
any automated tests during our development. Still, to substantiate
...
\footnotetext{ The examples are again taken from imperative asteroids and pure-asteroids.
...
More extensive debuggers like Hat exist and package and project managers like Cabal
and Stack also speed up development. However, In terms of game development specific tools
...
The games were altered to spawn different quantities of asteroids and
run for ten seconds with the ship constantly spinning and shooting one bullet per frame.
The results are rather illustrational as the differences between games were not controlled
very well: impure asteroids uses GLUT with the FPS limited to maximum of 59, because of issues
with Stack on Windows the Haskell games were run in the interpreter, hAsteroids renders
textures instead of drawing vectors like the other two and other minor differences.
The results can be seen in table \ref{tab:benchfps}.
We ran the test for 100 asteroids also in a virtual machine with Ubuntu,
where \inlinehs{stack build} works normally, and the results were very different:
46, 21 and 34 FPS respectively for imperative asteroids, hAsteroids and pure-asteroids.

\begin{table}
  \begin{tabularx}{\textwidth}{|r|llX|}
    \toprule
    Asteroid count & \textbf{impure asteroids} & \textbf{hAsteroids} & \textbf{pure-asteroids} \\
    \midrule
    10000 & 41 & 2   & 1   \\
    500   & 59 & 30  & 16    \\
    250   & 59 & 46  & 23    \\
...
Unfortunately, due to time constraints we did not diagnose the cause of these behaviours,
however, we can assume, what appears to be true in other fields and what is reflected
in the compiled benchmarks, that Haskell is mostly comparable to \cpp{} in performance,
and then as we expected, that apecs is faster than non-ECS data model for higher entity counts.

Worthy of noting is also the bit of profiling we did on the runs in Ubuntu.
...
GHC can do a lot of optimizing on its own and purely functional code is easier
...
imp asteroids does implement some extra stuff and has buttloads of comments,
but also does not have ufos
...
\caption{Comparing ship drawing in imperative asteroids and pure\babelhyphen{nobreak}asteroids.}
