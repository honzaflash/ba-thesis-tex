...
monads keeping state and access to I/O and the numerous \inlinehs{do} blocks.
This is certainly a valid approach and has its benefits as we discuss
further in chapter \ref{chptr:comparison} but for the second version of
...
Listing \ref{lst:events} shows the event package type \inlinehs{WorldEvents}. It
has an instance of \inlinehs{Monoid}, which allows us to collect the packages from
...
Related to expressiveness, is what we may call \say{functional elegance}.
Throughout the code base, we use functions like \inlinehs{map}, \inlinehs{filter},
\inlinehs{fold}, \inlinehs{traverse} and many others, that make the code brief
and save us time.
...
\label{chptr:comparison}

in this chapter we first compare, paying attention to what the promised benefits of FP were,
than summarize the pros and cons of Haskell
